...
git --no-pager log --format='%D' "${PR_BRANCH}" \
  ...
  | head -1
```
Walks the commit history of the PR branch and returns the first `origin/<name>` decoration encountered. If the result is empty, fall through to the next strategy.
